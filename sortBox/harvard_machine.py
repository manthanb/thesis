...
class HNM(nn.Module):
	...
	def __init__(self, N, W, networks, params, featureExtractorParams):
		...
		self.layer_ζ_1 = nn.Linear(12, 32)
		self.layer_ζ_2 = nn.Linear(32, 64)
		self.layer_ζ_3 = nn.Linear(64, 3)
	...
	def _alu_compute(self, P, image, gps):
		outPick = P[0][0]*self.networks[0].forward(image)
		outRed = P[0][1]*self.networks[1].forward(gps)
		outGreen = P[0][2]*self.networks[2].forward(gps)

		out = torch.cat((outPick, outRed, outGreen),1)
		return out
	...
	def forward(self, X, robotGps, read_weights, write_weights):
		...
		alu_input = self.read_head
		P = F.relu(self.layer_ζ_1(alu_input))
		P = F.relu(self.layer_ζ_2(P))
		P = F.softmax(self.layer_ζ_3(P))

		out = self._alu_compute(P, image, robotGps)
